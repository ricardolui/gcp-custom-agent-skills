...
> [!IMPORTANT]
> **Pre-execution Check**: Before running `gcloud` deployment or IAM configuration commands:
> 1. Ensure your `.env` exists with your project identifiers:
>    ```bash
>    cp .env.example .env
>    ```
...
>    ```
> 3. Use `${GCP_PROJECT_ID}` and `${GCP_REGION}` across all CLI invocations.
...
1. **Load Balancer IAP:** When users visit the custom domain, they authenticate against the Load Balancer's IAP Client ID (e.g., `123456789012-xxxx.apps.googleusercontent.com`). The Load Balancer generates and signs a JWT assertion.
2. **Cloud Run Native IAP:** The Cloud Run service itself is natively protected by a Google-managed system OAuth Client ID (e.g., `987654321098-xxxx.apps.googleusercontent.com`).
