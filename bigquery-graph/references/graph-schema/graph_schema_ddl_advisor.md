...
### Step 1: Analyze User Requirements & Data Profiling

1.  Identify the source BigQuery tables/views, their columns, and the intended
    relationships (nodes and edges).
2.  **Execute Data Profiling & Value Matching**: When column names differ across tables (e.g. `categoria` vs `cat_xpto`), profile distinct values and calculate **Jaccard Similarity** ($J \ge 0.80$ for shared categorical domains) and **Containment Ratio** ($C \ge 0.95$ for Primary Key $\rightarrow$ Foreign Key edge candidates).
3.  Determine if the user is building a standard **Property Graph** or a
    **Semantic Graph** (incorporating Measures, Descriptions, Synonyms).
4.  Check if the graph will be queried via **`GRAPH_EXPAND`** (flattened view).
...
### Step 3: Apply Schema Best Practices & Pre-Flight Topology Audit

1.  Consult **[best_practices.md](best_practices.md)**.
2.  **Execute Topology Pre-Flight Audit**:
    *   *Zero-Degree Check*: Verify all nodes have $\ge 1$ connected edge (no island nodes).
    *   *Multientity Symmetrical Edges*: Ensure facts with multiple entity foreign keys generate dedicated edges to all participating entities.
    *   *Canonical Network Decoupling*: Use static/current topological tables (`dim_network_topology`) for structural traversals (`*1..N`), keeping weekly snapshots in fact tables.
    *   *Unstructured AI Rule Binding*: Bridge LLM-extracted rules to structured programs via bridge tables (`rel_regla_programa`).
    *   *1-Hop Shortcuts*: Add direct edges for aggregated hierarchy reporting where appropriate.
3.  **Scope Properties**: Do **NOT** use `PROPERTIES ALL COLUMNS` or omit
    properties unless explicitly requested. Enforce `PROPERTIES (col1, col2)` to
    expose only necessary columns.
4.  **Enforce Safe Aliases**: Ensure every node and edge table has a clean,
    valid alias using `AS alias`. A valid alias must conform to the table name
    conventions in
    https://docs.cloud.google.com/bigquery/docs/tables#table_naming.
5.  **Check for Name Collisions**: If the user intends to use `GRAPH_EXPAND`,
...
2.  Provide a bulleted list summarizing the optimizations and corrections
    applied (e.g., "Resolved island node X by adding edge Y", "Scoped properties on Node Z").
